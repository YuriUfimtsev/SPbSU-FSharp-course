...
module LambdaInterpreter =
    ...
    let getNextVariable (character: char) = char (int (character) + 1)
    ...
    let getRightVariableName previousVariable newTerm =
        ...
        let rec processTheTerm term variablesCollection =
            match term with
            | Variable var ->
                if not (isVariableContainedInCollectionAsBound var variablesCollection) then
                    ((var, false) :: variablesCollection)
                else
                    variablesCollection
            | LambdaAbstraction(lambdaVariable, body) ->
                processTheTerm body ((lambdaVariable, true) :: variablesCollection)
            | Application(firstTerm, secondTerm) ->
                processTheTerm firstTerm variablesCollection
                @ processTheTerm secondTerm variablesCollection
        ...
        if not (isVariableContainedInCollectionAsFree previousVariable variablesCollection) then
            ...
            let rec loop variableName variablesList =
                if not (isVariableContainedInCollectionAsFree variableName variablesList) then
                    variableName
                else
                    loop (getNextVariable (variableName)) variablesList
    ...
    let rec performSubstitution previousVariable sourceTerm newTerm =
        ...
        | Application(firstTerm, secondTerm), term ->
            Application(
                performSubstitution previousVariable firstTerm term,
                performSubstitution previousVariable secondTerm term
            )
        | LambdaAbstraction(lambdaVariable, body), Variable _ when previousVariable = lambdaVariable ->
            LambdaAbstraction(lambdaVariable, body)
        | LambdaAbstraction(lambdaVariable, body), term ->
            let rightVariableName = getRightVariableName lambdaVariable term
            if rightVariableName <> lambdaVariable then
                LambdaAbstraction(
                    rightVariableName,
                    performSubstitution
                        previousVariable
                        (performSubstitution lambdaVariable body (Variable rightVariableName))
                        term
                )
            else
                LambdaAbstraction(lambdaVariable, performSubstitution previousVariable body term)
    ...
    let betaConversion expression =
        let rec recursiveBetaConversion expression isApplicationWithLambdaAbstractionFound =
            ...
            | Application(LambdaAbstraction(variable, body), term), _ ->
                performSubstitution variable body term, true
    ...
    let betaReduction expression =
        let rec loop lambdaExpression isApplicationWithLambdaAbstractionFound =
            if not isApplicationWithLambdaAbstractionFound then
                lambdaExpression
            else
