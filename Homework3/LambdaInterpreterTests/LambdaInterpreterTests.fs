...
// λs.s s
let selfApplicabilityCombinator () =
    LambdaAbstraction('s', Application(Variable 's', Variable 's'))

// (λx.y) ((λx.x x x) (λx.x x x))
let expressionForNormalStrategyTest () =
    Application(
...
let expressionForAlphaConversionTest () =
    Application(
        LambdaAbstraction(
            'x',
            LambdaAbstraction(
                ...
                LambdaAbstraction(
                    'z',
                    Application(Application(Application(Variable 'x', Variable 'y'), Variable 'z'), Variable 't')
                )
            )
        ),
...
// λ_.λz.y _ z t (_ != y)
let expectedResultFromAlphaConversionTest () =
    LambdaAbstraction(
        '_',
        LambdaAbstraction(
            'z',
...
let expressionForBoundVariablesReplacementTest () =
    Application(
        LambdaAbstraction(
            'z',
            LambdaAbstraction(
                'y',
                Application(
                    Application(Application(Variable 'x', Variable 'y'), Variable 'z'),
...
let expectedResultFromBoundVariablesReplacementTest () =
    LambdaAbstraction(
        '_',
        Application(
...
let expressionForThreeBetaConversionTest () =
    Application(
        Application(
            LambdaAbstraction(
                'x',
                LambdaAbstraction(
                    'r',
                    LambdaAbstraction('t', Application(Application(Variable 'r', Variable 't'), Variable 'x'))
                )
            ),
            Variable 'k'
        ),
        Application(LambdaAbstraction('p', Application(Variable 'p', Variable 'r')), Variable 't')
    )
// (λ_.t r _ k)
let expectedResultFromthreeBetaConversionTest () =
    LambdaAbstraction(
        '_',
        Application(Application(Application(Variable 't', Variable 'r'), Variable '_'), Variable 'k')
    )

[<Test>]
let ``Apply an IdCombinator and an arbitrary term, check that result is the second term`` () =
    betaReduction (Application(IdCombinator(), (Variable 'c')))
    |> should equal (Variable 'c')

    betaReduction (Application(IdCombinator(), (Variable 'x')))
...
let ``Apply a selfApplicabilityCombinator and an IdCombinator, check that result is the IdCombinator`` () =
    betaReduction (Application(selfApplicabilityCombinator (), IdCombinator()))
    |> should equal (IdCombinator())

[<Test>]
let ``Check that beta interpreter implements normal strategy`` () =
    betaReduction (expressionForNormalStrategyTest ())
...
let ``Check that alpha conversion is implemented`` () =
    ...
    | LambdaAbstraction(firstVariable,
                        LambdaAbstraction('z',
                                          Application(Application(Application(Variable 'y', Variable secondVariable),
                                                                  Variable 'z'),
                                                      Variable 't'))) when
        firstVariable = secondVariable && firstVariable <> 'y'
        ->
        true
    | _ -> false
    |> should be True
...
let ``Check that bound variables aren't replaced during beta reduction`` () =
    ...
    | LambdaAbstraction(firstVariable,
                        Application(Application(Application(Variable 'x', Variable secondVariable), Variable 'y'),
                                    LambdaAbstraction('z',
                                                      Application(Application(Variable 'r', Variable 'f'), Variable 'z'))))
        when firstVariable = secondVariable && firstVariable <> 'y'
        ->
        true
    | _ -> false
    |> should be True
...
let ``Apply three beta conversion`` () =
    ...
    | LambdaAbstraction(firstVariable,
                        Application(Application(Application(Variable 't', Variable 'r'), Variable secondVariable),
                                    Variable 'k'))
        when firstVariable = secondVariable && firstVariable <> 'r' && firstVariable <> 't'
        ->
        true
    | _ -> false
    |> should be True
